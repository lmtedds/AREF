...
export const setCookiePreferences = async (page: Page): Promise<void> => {
	...
	if(preferencesButtonEles.length !== 1) return Promise.reject(`Unable to find 1 cookie preference button ele: ${preferencesButtonEles.length}`);
	...
	if(!cookieMenu) return Promise.reject(`Unable to find 1 menu for cookies`);
	...
	if(popBottomButtonEles.length !== 1) return Promise.reject(`Unable to find the save settings button for ${name} cookies: ${popBottomButtonEles.length}`);
...
const turnOffCookieType = async (page: Page, cookieMenu: ElementHandle<Element>, name: string) => {
	...
	if(!cookieMenuItemEle) return Promise.reject(`Unable to find ${name} cookies menu entry`);
	...
	if(checkboxEles.length !== 1) return Promise.reject(`Can't find 1 ${name} cookie checkbox: ${checkboxEles.length}`);
