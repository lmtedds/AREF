...
const getHostListingSection = async (page: Page): Promise<ElementHandle<Element>> => {
	...
	const result = await page.evaluateHandle((hostName) => {
		...
		logger.assert(divs.length > 0, `Unable to find host listing section divs`);

		const theDiv = divs.find((div: HTMLElement) => {
			return div.innerText.startsWith(hostName);
		});
		logger.assert(theDiv, `Unable to find host listing section div`);
...
const getHostsNumListings = async (page: Page): Promise<number> => {
	...
	if(!listingsText) {
		logger.warn(`WARN: Unable to find any all listings text (presumably quick list has everything): ${listingsText}`);
...
const getHostReviewSection = async (page: Page): Promise<ElementHandle<Element> | undefined> => {
	...
	const reviewSection = await page.evaluateHandle(() => {
		...
		logger.assert(divs.length === 1, `Unable to find review section section header ${divs.length}`);
...
const parseHostReviewType = (text: string): "guests" | "hosts" | null => {
	...
	if(!match || match.length !== 3) {
		logger.error(`Unable to parse review tab button content: "${text}" ${JSON.stringify(match)}`);
	} else {
		switch(match[1]) {
			case "guests":
			case "hosts":
			...
			default:
				logger.error(`Unexpected review tab title`);
				break;
		}
	}
...
const getNumHostReviews = async (page: Page): Promise<IAirbnbHostNumReviews> => {
	...
	if(outerTabs.length === 0) {
		...
		if(!reviewSection) {
			logger.warn(`WARN: no reviews found on ${page.url()}`);
			return Promise.resolve({
				fromGuests: 0,
				fromHosts: 0,
			});
...
// 	logger.error(`for all review tabs need to "get more reviews" until there are no more`);
...
// 		logger.assert(tabHeaderDiv.length > 0, `Unable to find review tabs button`);

// 		const tabTypes: string[] = tabHeaderDiv.map((tab) => {
// 			const reviewType = parseHostReviewType(tab.innerText);
// 			logger.assert(reviewType, `unknown review type ${reviewType}`);
...
// 		// 	logger.assert(content.length === 1, `Found too many tab review content sub divs`);
