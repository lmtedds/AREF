...
export const findSurvey = async (page: Page): Promise<boolean> => {
	...
	if(forms.length > 1) throw new Error(`Too many forms found when looking for a survey: ${forms.length}`);
...
export const closeSurvey = async (page: Page): Promise<void> => {
	...
	if(closeXs.length !== 1) throw new Error(`Unable to find survey closing X: ${closeXs.length}`);
